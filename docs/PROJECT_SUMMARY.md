...
- 全特徴を使う多出力Ridgeは目的変数について線形なので、PCA-LIMEの予測は
  ...
  PCA次元だけでなく、近傍での予測変動BᵀΣBの有効ランク（`linear_q95`）である。
  係数二乗和の保持率（`coefficient_energy_retained`）は入力方向の変動量の違いを
  無視するので、説明の振る舞いの保持率として読まない。
- 局所低次元の理由は、q95を「出力分散の95%を担うクラス数−1」（`moving_classes`）と
  比べて判定する。確率が閾値に達するクラス数は一定の高確率クラスも含むので使わない。
  合成データのMLPではq95 ≈ moving−1で、「動くクラスが少ない」ことで説明できた。
